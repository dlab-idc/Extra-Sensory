...
def get_folds_list(fold_path):
    ...
    for i in range(0, 5):
        train_fold = _get_fold_train(i, fold_path)
        test_fold = _get_fold_test(i, fold_path)
...
def _get_single_fold_uuids_lst(fold_type, fold_num, fold_path):
    uuids_fold_lst = []
    fold_srcs = ['android', 'iphone']

    for fold_src in fold_srcs:
        with open(fold_path / f"fold_{fold_num}_{fold_type}_{fold_src}_uuids.txt", 'r') as fis:
            for line in fis:
                if line is not '':
                    uuid = line.strip()
...
def _get_fold_train(fold_num, fold_path):
    return _get_single_fold_uuids_lst("train", fold_num, fold_path)
